...
const server = createServer(async (req, res) => {
  try {
    ...
    /* ---- results + cost ---- */
    if (req.url === "/api/results.csv" && req.method === "GET") {
      const { targets } = await targetsStore.read();
      const state = await enrichment.read();
      const esc = (v) => {
      ...
      const header = "company,domain,contact_name,contact_title,contact_email,email_basis,linkedin,persona,headcount,segment,hook,timing_signal,summary,sources,researched_by";
      const lines = targets
        .map((t) => ({ t, st: state.companies[t.key] }))
        .filter(({ st }) => st?.last)
        .map(({ t, st }) => [
          t.name, t.domain, st.last.contactName, st.last.contactTitle,
          st.last.contactEmail, st.last.contactEmailBasis, st.last.contactLinkedin,
          st.last.contactPersona, st.last.headcount, st.last.segment, st.last.hook,
          st.last.timingSignal, st.last.summary, (st.sources || []).join(" "),
